...
class AutoComplete:
    ...
    def delete_char(self, sentence: str, ht):
    ...
    def character_addition(self, sentence, ht):  # THEE GOOD
        n = len(sentence) - 1
        res = []
    ...
    #####################################################################################
    def has_multiple_mismatches(self, subtext, subtextdict):
        """
    ...
    def find_mismatched_word_and_index(self, subtext, subtextdict):
        """
    ...
    def generate_possible_replacements(self, mismatched_word, subtext, subtextdict, end_word_index):
        """
    ...
    def find_most_suitable_lines_by_replace_char(self, subtext, subtextdict):
        """
        ...
        if self.has_multiple_mismatches(subtext, subtextdict):
            return []

        mismatched_word, start_index = self.find_mismatched_word_and_index(subtext, subtextdict)

        end_word_index = start_index + len(mismatched_word) - 1
        return self.generate_possible_replacements(mismatched_word, subtext, subtextdict, end_word_index)

    #####################################################################################################
    def find_most_five_suitable_lines(self, subtext, subtextdict):
        list_of_suitable_by_replace_char = self.find_most_suitable_lines_by_replace_char(subtext, subtextdict)
        list_of_suitable_by_delete_char = self.delete_char(subtext, subtextdict)
        list_of_suitable_by_add_char = self.character_addition(subtext, subtextdict)
    ...
    def create_five_autocompletedata_objects(self, most_five_suitable_lines, subtextdict):
        auto_complete_object = []
        for i in range(5):
            line_in_file = subtextdict[most_five_suitable_lines[i][0]][0]
            name_of_source_file = subtextdict[most_five_suitable_lines[i][0]][2]
            offset = subtextdict[most_five_suitable_lines[i][0]][1]
    ...
    def get_best_k_completion(self, subtext: str, subtextdict: dict):
        """
        ...
        if subtext in subtextdict.keys():
            ...
            most_five_suitable_lines = self.find_most_five_suitable_lines(subtext, subtextdict)
            autoCompleteData_list = self.create_five_autoCompleteData_objects(most_five_suitable_lines, subtextdict)
